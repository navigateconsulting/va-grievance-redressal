...
export class WebSocketService {
  ...
  createGrievanceRoom(grievance_room: string) {
    this.socket.nsp = constant.GRIEVANCE_NSP;
    this.socket.emit('join_room', grievance_room);
  }

  getGrievance(grievance_room: string) {
    this.socket.emit(constant.GRIEVANCE_URL, grievance_room);
    return Observable.create((observer) => {
      this.socket.on(constant.GRIEVANCE_LISTEN, (data) => {
        if (data) {
          observer.next(data);
        } else {
          observer.error('Unable To Reach Server');
  ...
  getAGrievance(grievance_room: string, grievance_id: string) {
    this.socket.emit(constant.AGRIEVANCE_URL, grievance_id, grievance_room);
    return Observable.create((observer) => {
      this.socket.on(constant.AGRIEVANCE_LISTEN, (data) => {
        if (data) {
          observer.next(data);
        } else {
          observer.error('Unable To Reach Server');
  ...
  leaveGrievanceRoom(grievance_room: string) {
    this.socket.emit('leave_room', grievance_room);
  }
